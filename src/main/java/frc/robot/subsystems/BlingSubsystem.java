...
public class BlingSubsystem extends SubsystemBase {
  ...
  public void update() {
    ...
    switch (mMode) {
      ...
      case AROUND -> {
        for (int i = 0; i < kLedLength; i++) {
          if (kAroundStripLength >= Math.abs(i - (mFrame * kAroundSpeedMultiplier % kLedLength)))
            setPixel(i, mix(Color.kBlack, mPrimary,
                Math.abs(i - (mFrame * kAroundSpeedMultiplier % kLedLength)) / kAroundStripLength));
          else
      ...
      case AROUND_SECONDARY_BG -> {
        for (int i = 0; i < kLedLength; i++) {
          if (kAroundStripLength >= Math.abs(i - (mFrame * kAroundSpeedMultiplier % kLedLength)))
            setPixel(i, mix(mSecondary, mPrimary,
                Math.abs(i - (mFrame * kAroundSpeedMultiplier % kLedLength)) / kAroundStripLength));
          else
            setPixel(i, mSecondary);
        }
      }
      case ERROR ->
      ...
      case WARNING ->
        setAllLeds(
            mix(Color.kYellow, Color.kOrange, Math.abs((float) mFrame % kAlertLength / kAlertLength - .5f) * 2f));
    }
  ...
  private void setPixel(int index, Color c) {
    ledBuffer.setRGB(index, (int) (c.red * kBrightness * 255.0), (int) (c.green * kBrightness * 255.0),
        (int) (c.blue * kBrightness * 255.0));
  }
  ...
  private void setPixel(int index, double r, double g, double b) {
    ledBuffer.setRGB(index, (int) (r * kBrightness * 255.0), (int) (g * kBrightness * 255.0),
        (int) (b * kBrightness * 255.0));
  }
}
